...
sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') CodeDeploy BeforeInstall starting..." >>"/home/ubuntu/log/deploy.log"
...
if [ -d "/home/ubuntu/env" ]; then
    ...
    rm -rf "/home/ubuntu/env" &&
        sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') Remove existing env." >>"/home/ubuntu/log/deploy.log" ||
        sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') Fail: Remove existing env." >>"/home/ubuntu/log/deploy.log"
else
    # Log a message if the env folder doesn't exist
    sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') No existing env to remove." >>"/home/ubuntu/log/deploy.log"
fi
...
if [ -d "/home/ubuntu/AWS_ECDjangoDeployer" ]; then
    ...
    rm -rf "/home/ubuntu/AWS_ECDjangoDeployer" &&
        sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') Remove existing repo." >>"/home/ubuntu/log/deploy.log" ||
        sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') Fail: Remove existing repo." >>"/home/ubuntu/log/deploy.log"
else
    # Log a message if the AWS_ECDjangoDeployer folder doesn't exist
    sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') No existing repo to remove." >>"/home/ubuntu/log/deploy.log"
fi

# log complete cicd BeforeInstall
sudo echo -e "$(date +'%Y-%m-%d %H:%M:%S') CodeDeploy BeforeInstall completed.\n" >>"/home/ubuntu/log/deploy.log"
